...
export default function VehicleCard({ v }: { v: Vehicle }) {
  ...
  const motor = v.features?.motor; // "300 kW"
  const seats = v.features?.seats; // 5
  const drivetrain = v.features?.drivetrain; // "AWD"
  const battery = v.features?.battery; // "87.7 kWh"

  const color = v.color ?? "—";
  const year = v.year;

  return (
    <Pressable onPress={() => router.push(`/(vehicles)/${v.id}` as Href)}>
    <View className="mb-4 rounded-3xl bg-[#111a24] overflow-hidden shadow-lg">
      ...
      <View style={{ borderRadius: 24, overflow: "hidden" }} className="relative">
        <Image
          source={{
            uri:
              v.image_url ||
              "https://tse1.mm.bing.net/th/id/OIP.BNr7COrS5-hwntskpdYHpQHaEK?rs=1&pid=ImgDetMain&o=7&rm=3",
          }}
        ...
        {/* overlay badge ở góc dưới phải ảnh */}
      ...
      <View className="px-5 pb-5">
        ...
        <View className="flex-row items-center gap-2 mt-1">
          {/* chấm màu cho color (nếu muốn ánh xạ hex thì thay style backgroundColor) */}
          <View
            style={{ width: 10, height: 10, borderRadius: 5, backgroundColor: "rgba(255,255,255,0.25)" }}
          />
          <Text className="text-stone-50" numberOfLines={1}>
            {color}
          </Text>
          {year ? (
            <>
        ...
        <View className="flex-row flex-wrap gap-x-4 gap-y-1.5 mt-3">
          ...
          {drivetrain && <Spec label={String(drivetrain)} f="cpu" />} {/* tạm dùng 'cpu' cho drivetrain */}
